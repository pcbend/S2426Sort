...
void TMidasEvent::Print(const char* option) const
{
   ...
   std::cout<<"  event id:       "<<std::hex << fEventHeader.fEventId << std::dec <<std::endl;
   std::cout<<"  trigger mask:   "<<std::hex << fEventHeader.fTriggerMask << std::dec<<std::endl;
   ...
   if((fEventHeader.fEventId & 0xffff) == 0x8000) {
      ...
      for(int i = 0; i < fBanksN * 4; i += 4) {
        ...
        if(option[0] == 'a' && (found != 0)) {
          //dealing with bank type 6 - hacky 
          for(int j = 0; j < bankLength; j++) {
            //if(j == highlight) {
					  //  std::cout<<ALERTTEXT<<std::hex((reinterpret_cast<uint32_t*>(pdata))[j],8)<<RESET_COLOR<<((j % 10 == 9) ? '\n' : ' ');
            //} else {
					    std::cout <<  std::hex
                        <<  (reinterpret_cast<uint32_t*>(pdata))[j] 
                        <<  ((j % 10 == 9) ? '\n' : ' ');
            //}
          }
        }
...
/*
         if(option[0] == 'a' && (found != 0)) {
            ...
            case 4: // TID_WORD
               for(int j = 0; j < bankLength; j++) {
                  if(j == highlight) {
							std::cout<<ALERTTEXT<<std::hex << reinterpret_cast<uint16_t*>(pdata) << std::dec <<RESET_COLOR<<((j % 10 == 9) ? '\n' : ' ');
                  } else {
							std::cout<<std::hex((reinterpret_cast<uint16_t*>(pdata))[j],4)<<((j % 10 == 9) ? '\n' : ' ');
                  }
               }
					std::cout<<std::endl;
               break;
            case 6: // TID_DWORD
               for(int j = 0; j < bankLength; j++) {
                  if(j == highlight) {
							...
							std::cout<<std::hex((reinterpret_cast<uint32_t*>(pdata))[j],8)<<((j % 10 == 9) ? '\n' : ' ');
                  }
               }
					std::cout<<std::endl;
               break;
